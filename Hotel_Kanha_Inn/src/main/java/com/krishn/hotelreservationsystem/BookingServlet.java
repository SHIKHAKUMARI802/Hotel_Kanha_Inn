...
public class BookingServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
		...
		//getting session values from index.jsp to store 
...
//			String roomType = req.getParameter("room_type");
			...
			String updateRoomQuery = "UPDATE rooms SET status = 'booked', check_in_date=?, check_out_date=? WHERE room_id = ?";
			PreparedStatement updateRoomStmt = con.prepareStatement(updateRoomQuery);
			updateRoomStmt.setDate(1, sqlInDate);
			updateRoomStmt.setDate(2, sqlOutDate);
			updateRoomStmt.setInt(3, room_id);
		     int rowsUpdated=updateRoomStmt.executeUpdate();
			if(rowsUpdated>0) {
			resp.sendRedirect("./Confirmed.jsp");
			}
			else {
			    out.print("<h3 style='color:red'>Error updating room status.</h3>");
			}
						}
		catch(Exception e) {
			e.printStackTrace();
